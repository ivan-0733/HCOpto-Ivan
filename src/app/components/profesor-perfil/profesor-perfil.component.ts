...
import { ProfesorService, ProfesorPerfil } from '../../services/profesor.service';
...
export class ProfesorPerfilComponent implements OnInit {
  ...
  alumnosUnicos: number | null = null;
  ...
  ngOnInit(): void {
    this.cargarPerfil();
    this.cargarPeriodoEscolar();
    this.initForm();
    this.cargarMaterias();
    this.cargarAlumnosUnicos();
  ...
  // Nueva función específica para alumnos únicos
  cargarAlumnosUnicos(): void {
    this.profesorService.obtenerAlumnosAsignados().subscribe({
      next: (alumnos) => {
        // Filtrar duplicados usando AlumnoInfoID
        const alumnosUnicos = [...new Map(alumnos.map(a => [a.AlumnoInfoID, a]))].values();
        this.alumnosUnicos = Array.from(alumnosUnicos).length;
      },
      error: (error) => {
        console.error('Error al cargar alumnos:', error);
        this.alumnosUnicos = null;
      }
    });
  }
  ...
  checkPersonalInfoFields(): void {
    if (this.isEditing) {
      ...
      if (nombreUsuario && nombreUsuario !== this.perfil?.NombreUsuario) {
        ...
        this.uniqueErrors = this.uniqueErrors.filter(error => error.field !== 'nombreUsuario');
      }
      ...
      if (correoElectronico && correoElectronico !== this.perfil?.CorreoElectronico) {
        ...
        this.uniqueErrors = this.uniqueErrors.filter(error => error.field !== 'correoElectronico');
      }
      ...
      if (telefonoCelular && telefonoCelular !== this.perfil?.TelefonoCelular) {
        ...
        this.uniqueErrors = this.uniqueErrors.filter(error => error.field !== 'telefonoCelular');
      }
    }
  }
  ...
  onSubmit(): void {
    ...
    if (actualizarContraseña) {
      ...
      this.profesorService.verificarPasswordActual(passwordActual).subscribe({
        next: (esValida) => {
          if (!esValida) {
...
private actualizarInformacionPersonal(
  ...
  this.profesorService.actualizarPerfil(datos).subscribe({
    next: (response) => {
      callback();
    },
    error: (error) => {
...
private actualizarPasswordUsuario(
  ...
  this.profesorService.actualizarPassword({
    passwordActual,
    nuevaPassword
  }).subscribe({
    next: (response) => {
...
private generateErrorId(): string {
  ...
  handleValidationErrors(errors: any[]): void {
    ...
    errors.forEach(error => {
      if (error.field === 'nombreUsuario') {
        if (error.type === 'unique' || error.message.includes('ya existe') || error.message.includes('ya está en uso')) {
          this.uniqueErrors.push({
            field: 'nombreUsuario',
      ...
      } else if (error.field === 'correoElectronico') {
        if (error.type === 'unique' || error.message.includes('ya existe') || error.message.includes('ya está registrado')) {
          this.uniqueErrors.push({
            field: 'correoElectronico',
            message: error.message
          });
      ...
      } else if (error.field === 'telefonoCelular') {
        if (error.type === 'unique' || error.message.includes('ya existe') || error.message.includes('ya está registrado')) {
          this.uniqueErrors.push({
            field: 'telefonoCelular',
            message: error.message
      ...
      } else if (error.field === 'passwordActual') {
  ...
  hasFieldError(fieldName: string): boolean {
    return [...this.personalInfoErrors, ...this.uniqueErrors].some(error => error.field === fieldName);
  }
  ...
  getFieldError(fieldName: string): string {
    const allErrors = [...this.personalInfoErrors, ...this.uniqueErrors];
    const error = allErrors.find(error => error.field === fieldName);
    return error ? error.message : '';
  }

  /**
   * Verifica si hay errores de campos específicos
   */
  hasAnyFieldErrors(): boolean {
    return this.personalInfoErrors.length > 0 || this.uniqueErrors.length > 0;
  }
